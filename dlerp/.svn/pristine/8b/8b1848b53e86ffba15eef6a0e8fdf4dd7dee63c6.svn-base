(function()
{
    return function()
    {
        ...
        // User Script
        this.registerScript("DLF_AGREEMENT.xfdl", function() {
        ...
        this.fnSetEvent = function() {
        	...
        	// 그리드 초기화
        	this.gfnGridInit(this.dxGrid, this.dsList, "DL", "DLF_AGREEMENT_1");
        	this.gfnGridInit(this.dxGridSub, this.dsListSub, "DL", "DLF_AGREEMENT_2");
        	...
        	for (var i = 0; i < this.dsListSub.rowcount; i++) {
        		...
        		switch(flag) {
        			case "I":
        				...
        				this.dsInsert.setColumn(nrow, "AM_SELLING_HANDLE", this.dsListSub.getColumn(i, "처리금액"));
        			...
        			case "U":
        				...
        				this.dsUpdate.setColumn(nrow, "AM_SELLING_HANDLE", this.dsListSub.getColumn(i, "처리금액"));
        ...
        this.fnAmountChk = function()
        {
        	...
        	for (var i = 0; i < this.dsListSub.rowcount; i++) {
        		if (this.gfnIsNull(this.dsListSub.getColumn(i, "처리금액"))){
        			continue;
        		}
        ...
        this.fnSbAM_Sum = function()
        {
        	...
        	var amTotalVal = nexacro.toNumber(this.dsListSub.getColumn(this.dsListSub.rowposition, "AM_SUPPLY"), 0) + nexacro.toNumber(this.dsListSub.getColumn(this.dsListSub.rowposition, "AM_VAT"), 0);
        	this.dsListSub.setColumn(this.dsListSub.rowposition, "AM_TOTAL", amTotalVal);
        }
        ...
        this.fnSbAM_DIVISION = function()
        {
        	...
        	var procAm = this.dsListSub.getColumn(this.dsListSub.rowposition, "처리금액");
        	...
        	if(procAm > 0){
        		...
        		//안분 비율구하기
        		_AM_DIVISION = nexacro.toNumber(procAm, 0) / nexacro.toNumber(this.dsList.getColumn(0, "AM_AMOUNT"), 0);

        		//공급가, 부가세 구하기
        		_AM_SUPPLY = nexacro.floor(nexacro.toNumber(procAm, 0) * 10 / 11);
        		_AM_VAT = nexacro.toNumber(procAm, 0) - nexacro.toNumber(_AM_SUPPLY, 0);

        		this.dsListSub.setColumn(this.dsListSub.rowposition, "AM_SUPPLY", _AM_SUPPLY);
        		this.dsListSub.setColumn(this.dsListSub.rowposition, "AM_VAT", _AM_VAT);

        		if(this.dsListSub.getColumn(this.dsListSub.rowposition, "TY_NAPIP1") == "잔금"){
        			for (var i = 0; i < this.dsListSub.rowcount; i++) {
        				if(this.gfnIsNull(this.dsListSub.getColumn(i, "처리금액"))) { continue; }

        				am_totalSupply += nexacro.toNumber(this.dsListSub.getColumn(i, "AM_SUPPLY"), 0);
                        am_totalVat += nexacro.toNumber(this.dsListSub.getColumn(i, "AM_VAT"), 0);
        			}

        			gap_Supply = nexacro.toNumber(this.dsList.getColumn(0, "AM_SUPPLY"), 0) - nexacro.toNumber(am_totalSupply, 0);
        			gap_vat = nexacro.toNumber(this.dsList.getColumn(0, "AM_VAT"), 0) - nexacro.toNumber(am_totalVat, 0);

        			_AM_SUPPLY += nexacro.toNumber(gap_Supply, 0);
        			_AM_VAT += nexacro.toNumber(gap_vat, 0);
        		}

        		this.dsListSub.setColumn(this.dsListSub.rowposition, "AM_SUPPLY", _AM_SUPPLY);
        		this.dsListSub.setColumn(this.dsListSub.rowposition, "AM_VAT", _AM_VAT);
        		this.dsListSub.setColumn(this.dsListSub.rowposition, "RT_DIVISION", nexacro.toNumber(_AM_DIVISION, 0) * 100);
        		this.dsListSub.setColumn(this.dsListSub.rowposition, "처리금액", nexacro.toNumber(_AM_SUPPLY, 0) + nexacro.toNumber(_AM_VAT, 0));
        	}
        }
        ...
        this.dsListSub_onvaluechanged = function(obj,e)
        {
        	if(e.oldvalue != e.newvalue)
        	{
        		...
        		if(e.columnid == "AM_SUPPLY") {
        			...
        			this.fnSbAM_Sum();
        		}else if(e.columnid == "처리금액") {
        			this.fnSbAM_DIVISION();
        			this.fnSbAM_Sum();
        		}
        	}
        };
         ...
         this.fnCopy = function(obj,e) {
        	...
        	this.gfnFormOpen(this.FormInfo.CD_MODULE, "DLF_DLGSIZEDIVCOPY", "fnCopyCallback", param);
        }
        ...
        this.loadIncludeScript("DLF_AGREEMENT.xfdl");
