(function()
{
    return function()
    {
        ...
        this.on_create = function()
        {
            ...
            obj.set_taborder("21");
            obj.set_text("[청구요청건]만 협력업체가 기성청구 가능하며 현장에서 기성작업 가능, 기성당회 입력 ");
            obj.set_color("red");
            obj.set_cssclass("sta_WF_SchLabel");
            this.divSearch.addChild(obj.name, obj);
        ...
        // User Script
        this.registerScript("DWB_HDGISUNGMONTH_M_DETAIL.xfdl", function() {
        this.YN_B2B = "";
        ...
        this.fnSetEvent = function() {
        	...
        	// 그리드 초기화
        	this.gfnGridInit(this.dxGrid, this.dsList, "DW", "DWB_HDGISUNGMONTH_M_DETAIL", "DS_COST", "GR_COST");
        ...
        */
        		...
        		switch (colnm){
        			...
        			case "QN_TTHADOGS" :
        				...
        				if(this.dsList.getColumn(row, "YN_LSUNIT") != "Y"){
        					...
        					/*var qn_hadogs = nexacro.Decimal(this.dsList.getColumn(row, "QN_ALL_TTHADOGS"));*/

        					qn_hadogs.subDecimal(nexacro.Decimal(this.dsList.getColumn(row, "PRE_QN_TTHADOGS"))); // 감산
        					// QN_HADOGS 선택업체	기성당회	수량 =  QN_ALL_TTHADOGS (공동수급사) 전체	기성누계 수량 -  PRE_QN_TTHADOGS 선택업체	기성전회 수량
        					// QN_HADOGS =  QN_ALL_TTHADOGS - PRE_QN_TTHADOGS
        					...
        					// 공동수급사 전체기성-금액
        					var am_all_tthadogs = nexacro.Decimal(this.dsList.getColumn(row, "AM_HADOGS"));

        					am_all_tthadogs.addDecimal(nexacro.Decimal(this.dsList.getColumn(row, "AM_ALL_TTHADOGS_ORI")));

        					this.dsList.setColumn(row, "AM_ALL_TTHADOGS", nexacro.floor(am_all_tthadogs));

        					// 공동수급사 전체잔여-금액
        					var j_am_all_tthadogs = nexacro.Decimal(this.dsList.getColumn(row, "AM_TT_HADO"));

        					j_am_all_tthadogs.subDecimal(nexacro.Decimal(this.dsList.getColumn(row, "AM_ALL_TTHADOGS")));

        					this.dsList.setColumn(row, "J_AM_ALL_TTHADOGS", nexacro.floor(j_am_all_tthadogs));
        			...
        			case "QN_HADOGS" :
        				if(this.dsList.getColumn(row, "YN_LSUNIT") != "Y"){
        					...
        					this.dsList.setColumn(row, "QN_TTHADOGS", nexacro.floor(qn_tthadogs, 3));
        					...
        					// 공동수급사 전체기성-수량
        					var qn_all_tthadogs = nexacro.Decimal(this.dsList.getColumn(row, "QN_HADOGS"));

        					qn_all_tthadogs.addDecimal(nexacro.Decimal(this.dsList.getColumn(row, "QN_ALL_TTHADOGS_ORI")));


        					// QN_ALL_TTHADOGS (공동수급사) 전체	기성누계 수량

        					this.dsList.setColumn(row, "QN_ALL_TTHADOGS", nexacro.floor(qn_all_tthadogs, 3));
        					...
        					// 공동수급사 전체잔여-수량
        					var j_qn_all_tthadogs = nexacro.Decimal(this.dsList.getColumn(row, "QN_TT_HADO"));

        					j_qn_all_tthadogs.subDecimal(nexacro.Decimal(this.dsList.getColumn(row, "QN_ALL_TTHADOGS")));

        					this.dsList.setColumn(row, "J_QN_ALL_TTHADOGS", nexacro.floor(j_qn_all_tthadogs, 3));
        ...
        // 		}
        		...
        		// (공동수급사) 전체기성 기성누계 수량 > 계약전체	수량
        		if(nexacro.floor(this.dsList.getColumn(row, "QN_ALL_TTHADOGS"), 3) > nexacro.floor(this.dsList.getColumn(row, "QN_TT_HADO"), 3)){

        			this.dsList.setColumn(row, "QN_ALL_TTHADOGS", nexacro.floor(this.dsList.getColumn(row, "QN_TT_HADO"), 3));


        			var qn_tthadogs = nexacro.Decimal(this.dsList.getColumn(row, "QN_ALL_TTHADOGS"));
        			var qn_tt_hado = nexacro.Decimal(this.dsList.getColumn(row, "QN_TT_HADO"));

        			//선택업체 기성누계 수량 = (공동수급사) 전체기성 기성누계 수량 - 전체기성(초기값) 수량
        			//qn_tthadogs.subDecimal(nexacro.Decimal(this.dsList.getColumn(row, "QN_ALL_TTHADOGS_ORI")));
        			qn_tt_hado.subDecimal(nexacro.Decimal(this.dsList.getColumn(row, "QN_ALL_TTHADOGS_ORI")));

        			//this.dsList.setColumn(row, "QN_TTHADOGS", nexacro.floor(qn_tthadogs, 3));
        			this.dsList.setColumn(row, "QN_HADOGS", qn_tt_hado);

        			var pre_qn_tthadogs  = nexacro.Decimal(this.dsList.getColumn(row, "PRE_QN_TTHADOGS"));
        			pre_qn_tthadogs.addDecimal(nexacro.Decimal(this.dsList.getColumn(row, "QN_HADOGS")));
        			this.dsList.setColumn(row, "QN_TTHADOGS", pre_qn_tthadogs);

        			// (공동수급사) 전체잔여	잔여기성 수량
        			this.dsList.setColumn(row, "J_QN_ALL_TTHADOGS", 0);
        ...
        // 		}
        		...
        		if((nexacro.floor(this.dsList.getColumn(row, "AM_HADOGS"), 3) > nexacro.floor(this.dsList.getColumn(row, "AM_TT_HADO"), 3)) && this.dsList.getColumn(row, "AM_TT_HADO") > 0){
        			...
        			am_hadogs.subDecimal(nexacro.Decimal(this.dsList.getColumn(row, "PRE_AM_TTHADOGS")));
        			if(this.FormInfo.CD_MODULE == "DWB"){
        ...
        */
        ...
        this.loadIncludeScript("DWB_HDGISUNGMONTH_M_DETAIL.xfdl");
