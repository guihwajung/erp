(function()
{
    return function()
    {
        ...
        this.fnSetExtendButton = function() {
        	...
        	this.btnNotice = this.gfnFormButtonAdd("btnNotice", "fnNotice");		// 보고서주의사항 버튼
        ...
        this.fnSetParameter = function() {
        	...
        	this.dsAutoSlip.addColumn("DT_REQUEST", "string");
        	this.dsAutoSlip.addColumn("CD_DEPT_BH", "string");
        	...
        	this.dsUpdateYnCtrl.addColumn("CD_DEPT_ANCT", "string");	// [1] 2024-08-05 보고서용 귀속부서 항목 관리 시작
        ...
        this.fnSave = function() {
        	...
        	for (var i = 0; i < this.dsList.rowcount; i++) {
        		...
        		case "D":
        			...
        			this.dsUpdateYnCtrl.setColumn(nrow, "CD_DEPT_ANCT" , this.dsList.getColumn(i, "CD_DEPT_ANCT")); // [1] 2024-08-05 보고서용 귀속부서 항목 관리 시작
        ...
        this.fnNotice = function() {
        	var param = {};
        	param.GUBUN = "RPT";
        	this.gfnFormOpen(this.FormInfo.CD_MODULE, "DAB_BZTP_RULE_RPT_DLG", "", param);
        }
        ...
        this.fnReport = function() {
        	...
        	param.CD_DEPT         = this.dsList.getColumn(nrow, "CD_DEPT_ANCT");		// [1] 2024-08-05 보고서용 귀속부서 항목 관리 시작

        	var DS_APRV_NAME = this.dsList.getColumn(nrow, "DS_APRV_NAME");   // 공통코드에서 가져오게 수정
        	var formName = "DAB_BZTP_RPT_" + DS_APRV_NAME + "_DLG";
        	this.gfnFormOpen(this.FormInfo.CD_MODULE, formName, "fnPlanCallback", param);
        ...
        this.fnAprv = function(obj, e) {
        	...
        	for (var i = 0; i < this.dsList.rowcount; i++) {
        		if (this.dsList.getColumn(i, "YN_CTRL") == "Y" && this.dsList.getColumn(i, "YN_OVER") == "Y") {
        			this.gfnAlert("집행금액이 예산을 초과하는 출장보고서가 포함되어 있습니다.\n예산통제 여부를 변경한 후 전자결재를 진행할 수 있습니다.");
        			this.dsList.filter("");
        	...
        	var msgchk = "※ 주의";
        	for (var i = 0; i < this.dsList.rowcount; i++) {
        		if (this.dsList.getColumn(i, "YN_IOUTBRUFLGTNOT_NOT_APLT") == "Y") {
        			msgchk += "\n   [입출국항공료의 예산 대비 집행금액을 입력하지 않았습니다.]"
        		}
        		if (this.dsList.getColumn(i, "YN_ETCNOT_NOT_APLT") == "Y") {
        			msgchk += "\n   [잡비의 예산 대비 집행금액을 입력하지 않았습니다.]"
        		}
        		if (this.dsList.getColumn(i, "YN_IOUTBRUFLGTNOT_CNT") == "N") {
        			msgchk += "\n   [입출국항공료/수수료가 각각 입력되지 않았습니다.]"
        		}
        		//msgchk += "\n\n   전자결재를 진행하시겠습니까?"
        	}
        	// 예산 미집행 알림
        	for (var i = 0; i < this.dsList.rowcount; i++) {
        		if (this.dsList.getColumn(i, "YN_IOUTBRUFLGTNOT_NOT_APLT") == "Y" || this.dsList.getColumn(i, "YN_ETCNOT_NOT_APLT") == "Y" || this.dsList.getColumn(i, "YN_IOUTBRUFLGTNOT_CNT") == "N") {
        			yn_chk = "Y";
        		}
        	}

        	var msg = "출장보고서(" + person + ")에 대하여\n" + obj.text + "하시겠습니까?";
        	for (var i = 0; i < this.dsList.rowcount; i++) {
        ...
        // 	// 출장일수 30일 초과 체크 (30일 초과일수에 대한 숙박비,식비,잡비는 10분의 7로 지급함)
        // 	var DT_BZTP = this.gfnGetDiffDate(DT_BZTP_FROM, DT_BZTP_TO) + 1;
        // 	var msg30over = "";
        // 	if (DT_BZTP > 30)
        // 	{
        // 		msg30over = "※ 출장일수가 " + this.gfnAppendComma(DT_BZTP) + "일로 30일을 초과한\n   숙박비, 식비, 잡비는 10분의 7로 지급됩니다.";
        // 		msg30over += "\n\n" + msg;
        // 		msg = msg30over;
        // 	}
        	...
        	this.gfnConfirm(msg, function(strId, isOk) {
        		...
        		var TITLE = locale === "[해외] " ? this.gfnGetConfig("DA" , "TITLE_RPT_OVERSEAS") : this.gfnGetConfig("DA" , "TITLE_RPT_DOMESTIC");
        		...
        		var initParam = {
        			...
        			TT_DOC: TITLE + DS_BZTP, // locale + "출장보고서 (" + person + ")",
        ...
        this.fnSlipPubCallback = function(id, val) {
        	...
        	for (var i = 0; i < this.dsList.rowcount; i++) {
        		...
        		this.dsAutoSlip.setColumn(nrow, "DT_REQUEST"   , result.DT_REQUEST);
        		this.dsAutoSlip.setColumn(nrow, "CD_DEPT_BH"   , this.AuthClient.CD_DEPT);
        ...
        this.fnProofFileValid = function(CD_CORP, SN_BZTPSEQ, ID_SABUN_BZTP, TY_BZTP) {
            ...
            trace(" fnProofFileValid=>" + SN_BZTPSEQ);
        ...
        // 		var CNT_FILE = this.dsListRptProof.getColumn(i, "CNT_FILE");
        		...
        		var DS_PMT_BIGO = this.dsListRptProof.getColumn(i, "DS_PMT_BIGO");	// 비용명
        		var NO_FILE = this.dsListRptProof.getColumn(i, "NO_FILE");			// 증빙갯수
        		var YN_PMT_TYPE = this.dsListRptProof.getColumn(i, "YN_PMT_TYPE");  // 결제구분 증빙필수여부 (잡비제외 모두)
        		var YN_PMT_MEAN =  this.dsListRptProof.getColumn(i, "YN_PMT_MEAN"); // 지불수단 증빙필수 여부(법인카드 제외 모두)

        		var DT_PMT = this.dsListRptProof.getColumn(i, "DT_PMT");

        		// 잡비와 법인카드를 제외하고 모든 증빙 체크
        		if (YN_PMT_TYPE == "Y" && YN_PMT_MEAN == "Y" && NO_FILE == 0) {
        			cnt++;
        			msg =  msg + cnt + "." + DS_PMT_BIGO + "의 증빙자료가 누락되었습니다."
        			msg =  msg + "\n" +
        	...
        	{
        		this.gfnAlert( cnt + "건의 증빙정보가 누락되었습니다.\n" + msg);
        			return false;
        	}
        //
