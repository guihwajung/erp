(function()
{
    return function()
    {
        ...
        {
            this.set_name("form");
            this.set_titletext("장기자금계획/실적조회");
            ...
            obj._setContents("<ColumnInfo><Column id=\"TARGET\" type=\"STRING\" size=\"256\"/><Column id=\"SP\" type=\"STRING\" size=\"256\"/></ColumnInfo><Rows><Row><Col id=\"TARGET\">select</Col><Col id=\"SP\">DFAPR_RESULTL_SELECT</Col></Row></Rows>");
            this.addChild(obj.name, obj);
        ...
        // User Script
        this.registerScript("DFA_RESULTL.xfdl", function() {
        ...
        this.fnSetEvent = function() {
        	...
        	// 그리드 초기화
        	this.gfnGridInit(this.dxGrid, this.dsList, "DF", "DFA_RESULTL");
        ...
        this.fnOpenDfaSuji = function(nrow, ncell) {
        	...
        	var month = colnm.toString().substr(3);	// 월 또는 TOT
        	...
        	if (this.gfnTrim(month) == "TOT") {
        		param.FR_DATE = this.gfnGetFirstDate(ymPlan.substr(0, 4)+"01");
        		param.TO_DATE = this.gfnGetLastDate(ymPlan);
        	} else {
        		param.FR_DATE = this.gfnGetFirstDate(ymPlan.substr(0, 4)+month);
        		param.TO_DATE = this.gfnGetLastDate(ymPlan.substr(0, 4)+month);
        	}
        ...
        this.loadIncludeScript("DFA_RESULTL.xfdl");
