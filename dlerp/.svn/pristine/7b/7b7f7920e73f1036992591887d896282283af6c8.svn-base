(function()
{
    return function()
    {
        ...
        // 	this.dxGrid.setCellProperty("Body",colSUN,"displaytype","decoratetext");
        	...
        	var sGetProperty = this.dxGrid.getCellProperty( "body", colMON, "cssclass" );

        	var sSetPropertyMON = sGetProperty.replace(": '')", ": MON_HOLIDAY==true || MON_HOLIDAY=='true' ? 'Red' : '')");
        	var sSetPropertyTUE = sGetProperty.replace(": '')", ": TUE_HOLIDAY==true || TUE_HOLIDAY=='true' ? 'Red' : '')");
        	var sSetPropertyWED = sGetProperty.replace(": '')", ": WED_HOLIDAY==true || WED_HOLIDAY=='true' ? 'Red' : '')");
        	var sSetPropertyTHU = sGetProperty.replace(": '')", ": THU_HOLIDAY==true || THU_HOLIDAY=='true' ? 'Red' : '')");
        	var sSetPropertyFRI = sGetProperty.replace(": '')", ": FRI_HOLIDAY==true || FRI_HOLIDAY=='true' ? 'Red' : '')");

        	this.dxGrid.setCellProperty( "body", colMON , "cssclass" , sSetPropertyMON );
        	this.dxGrid.setCellProperty( "body", colTUE , "cssclass" , sSetPropertyTUE );
        	this.dxGrid.setCellProperty( "body", colWED , "cssclass" , sSetPropertyWED );
        	this.dxGrid.setCellProperty( "body", colTHU , "cssclass" , sSetPropertyTHU );
        	this.dxGrid.setCellProperty( "body", colFRI , "cssclass" , sSetPropertyFRI );
        ...
        this.fnMakeCol = function(){
        	...
        	this.dsList.addColumn("MON_HOLIDAY", "boolean");// MON	월
        	this.dsList.addColumn("TUE_HOLIDAY", "boolean");// TUE	화
        	this.dsList.addColumn("WED_HOLIDAY", "boolean");// WED	수
        	this.dsList.addColumn("THU_HOLIDAY", "boolean");// THU	목
        	this.dsList.addColumn("FRI_HOLIDAY", "boolean");// FRI	금
        ...
        this.fnGetNameList = function(dataset){
        	...
        	var sNmDAY = this.gfnIsNull(dataset.getColumn(0, "DS_REMARK")) ? "" : dataset.getColumn(0, "DS_REMARK");        // 휴일명
        	trace("fnGetNameList>>sNmDAY =>" + sNmDAY);
        ...
        // 	trace("fnGetNameList>>sDD", sDD);
        	...
        	var sLine2 = this.gfnIsNull(sDT_DAY) ? "" : (sDS_HNAME == "" ? "" : "("+nCnt+"명)");
        	    sLine2 = this.gfnIsNull(sNmDAY) ? this.fnLpad(sLine2, 20, " ") : this.fnLpad(sNmDAY + sLine2, 16, " ");
        	...
        	//trace("fnGetNameList>>sStr =>" + sStr);
        	rtnObj.text = sStr;
        	rtnObj.tooltiptext = sNameList;
        	rtnObj.holiday = this.gfnIsNull(sNmDAY) ? false : true ;
