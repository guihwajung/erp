(function()
{
    return function()
    {
        ...
        this.on_create = function()
        {
            ...
            obj._setContents("<ColumnInfo><Column id=\"TARGET\" type=\"STRING\" size=\"256\"/><Column id=\"SP\" type=\"STRING\" size=\"256\"/></ColumnInfo><Rows><Row><Col id=\"TARGET\">select</Col><Col id=\"SP\">DBSPR_COST_FLU_SELECT</Col></Row></Rows>");
            this.addChild(obj.name, obj);
        ...
        // User Script
        this.registerScript("DBS_COST_FLU.xfdl", function() {
        ...
        this.fnSetEvent = function() {
        	this.gfnGridInit(this.dxGrid, this.dsList, "DB", "DBS_COST_FLU");  // pForm.gfnGridInit = function(objGrid, objDs, cd_system, id_grid, tree_column, level_column, is_manual)
        	var nCol = this.dxGrid.getBindCellIndex("body","DS_DIFF");
        	nCol = this.dxGrid.mergeContentsCell("head",0,nCol,0,nCol+1,nCol,false);
        	this.dxGrid.setCellProperty( "head", nCol, "textAlign", "center");

        	this.dxGrid.setCellProperty( "body", nCol, "subsumtext", "expr:nexacro.toNumber(AM_DIFF,0) == 0 ? '':(nexacro.toNumber(AM_DIFF,0) > 0 ? '△':'▼')");

        	this.dxGrid.setCellProperty( "summary", nCol, "text", "expr:nexacro.toNumber(dataset.getCaseSum('"+this.ucFlag+" != \"#\"','AM_DIFF')) == 0 ? '':(nexacro.toNumber(dataset.getCaseSum('"+this.ucFlag+" != \"#\"','AM_DIFF')) > 0 ? '△':'▼')");
        	this.dxGrid.setCellProperty( "summary", nCol, "textAlign", "center");

        	nCol = this.dxGrid.getBindCellIndex("body","DS_TYPE");
        	nCol = this.dxGrid.mergeContentsCell("head",0,nCol,0,nCol+1,nCol,false);
        	this.dxGrid.setCellProperty( "head", nCol, "textAlign", "center");

        	this.dxGrid.setCellProperty( "body", nCol, "subsumtext", "소계");

        	nCol = this.dxGrid.mergeContentsCell("summary",0,nCol,0,nCol+1,nCol,false);
        	this.dxGrid.setCellProperty( "summary", nCol, "textAlign", "center");
        ...
        this.fnPopupCallBack = function(svcID, value) {
        	...
        	if( !this.gfnIsNull(value) ){
        		...
        		var inData      = "select=dsSelect";
        		var reportpath  = "/db/dbs/DBS_COST_FLU.ozr";
        ...
        this.loadIncludeScript("DBS_COST_FLU.xfdl");
