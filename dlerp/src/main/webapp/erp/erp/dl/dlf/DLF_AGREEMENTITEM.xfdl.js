(function()
{
    return function()
    {
        ...
        // User Script
        this.registerScript("DLF_AGREEMENTITEM.xfdl", function() {
        ...
        this.fnSetEvent = function() {
        	// 그리드 초기화
        	this.gfnGridInit(this.dxGrid, this.dsList, "DL", "DLF_AGREEMENTITEM");
        	this.gfnGridInit(this.dxGridSub, this.dsListSub, "DL", "DLF_AGREEMENTITEM_2");
        ...
        */
        ...
        // 공급가,부가세 계산
         this.fnSbAM_DIVISION = function() {
        	...
        	var _ucTOTAL = this.dsListSub.getColumn(this.dsListSub.rowposition, "AM_CAL");
        	...
        	if(_ucTOTAL > 0){
        		...
        		//공급가, 부가세 구하기
        		_AM_SUPPLY = nexacro.floor(nexacro.toNumber(_ucTOTAL, 0) * 10 / 11);
        		_AM_VAT = nexacro.toNumber(_ucTOTAL, 0) - nexacro.toNumber(_AM_SUPPLY, 0);
        		...
        		if(this.dsListSub.getColumn(this.dsListSub.rowposition, "TY_NAPIP") == "잔금"){
        			for (var i = 0; i < this.dsListSub.rowcount; i++) {
        				am_totalSupply += this.dsListSub.getColumn(i, "AM_SUPPLY");
                        am_totalVat += this.dsListSub.getColumn(i, "AM_VAT");
        			}

        			if(this.YN_TOTCONTRACT == "Y"){
        				for (var i = 0; i < this.dsList.rowcount; i++) {
        					gap_Supply += nexacro.toNumber(this.dsList.getColumn(i, "AM_SUPPLY"), 0);
        					gap_vat += nexacro.toNumber(this.dsList.getColumn(i, "AM_VAT"), 0);
        				}

        				gap_Supply -= nexacro.toNumber(am_totalSupply, 0);
        				gap_vat -= nexacro.toNumber(am_totalVat, 0);
        			}else{
        				gap_Supply = nexacro.toNumber(this.dsList.getColumn(this.dsList.rowposition, "AM_SUPPLY"), 0) - nexacro.toNumber(am_totalSupply, 0);
        				gap_vat = nexacro.toNumber(this.dsList.getColumn(this.dsList.rowposition, "AM_VAT"), 0) - nexacro.toNumber(am_totalVat, 0);
        			}
        ...
        this.fnItemReceipt = function(obj,e) {
        	...
        	this.gfnFormOpen(this.FormInfo.CD_MODULE, "DLF_DLGCONTRACT", "fnItemReceiptCallback", param);
        }
        ...
        this.loadIncludeScript("DLF_AGREEMENTITEM.xfdl");
