...
pForm.gfnGetFileConfig = function(uploadUrl, downloadUrl, deleteUrl) {
	var _contextPath = nexacro.getEnvironmentVariable("evContextPath");
	if (_contextPath == undefined) {
		_contextPath = "/"
	}
	
	if(this.gfnIsNull(uploadUrl)) uploadUrl = _contextPath + "file/saveFile.do";
	if(this.gfnIsNull(downloadUrl)) downloadUrl = _contextPath + "file/downloadFile.do";
	if(this.gfnIsNull(deleteUrl)) deleteUrl = _contextPath + "file/deleteFile.do";
