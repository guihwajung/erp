...
pForm.gfnIsSSN = function(sJuminNo)
{
	...
	// 주민번호 개편에 따른 2020년10월부터 임의번호 발급으로 규칙없음.
	if (birth.getYear() > 2020 || (birth.getYear() >= 2020 && birth.getMonth() >= 9)) {
		return true;
	}
...
pForm.gfnIsFrnrIdNo = function(strNo)
{
	...
	if (strNo.length != 13 || !isNumber(Number(strNo))) return false;
...
pForm.gfnIsEmail = function(strValue)
{
	var retVal = false;
	var sTmp = "";
	var sRegExp = "[a-z0-9]+[a-z0-9_.,]+@[a-z0-9]+[a-z0-9.,-]+\\.[a-z0-9]+";
