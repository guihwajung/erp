...
if (!nexacro.Form) {
	...
	_pFormBase._waitCursor = function (wait_flag, context) {
		...
		if (this.parent != null && (this.parent.id == "frameLeft" || this.parent.id == "frameMain")) {
			if (this.getOwnerFrame() != null && this.getOwnerFrame().form != null) {
				var formID = this.getOwnerFrame().form._unique_id;
				
				if (wait_flag == true) {
					if (this.parent._ref_comm == null) this.parent._ref_comm = 0;
					...
					this.parent._ref_comm++;
				}
				else {
					if (this.parent._ref_comm > 0) {
						this.parent._ref_comm--;
					}
					
					if (document.getElementById("loading-container_"+formID) != null) {
						if (this.parent._ref_comm <= 0) {
							this.parent._ref_comm = 0;
